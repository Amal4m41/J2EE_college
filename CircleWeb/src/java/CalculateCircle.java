...
//fetching the data from the request object by specifying the name.
            String circle_prop=request.getParameter("circle_prop");
...
//extracting the value for the parameters specified in the web.xml(giving global access to these values from all servlets with in the application)
            ...
            Double PI=Double.valueOf(config.getInitParameter("PI"));  
            
            String result;
            
            if(circle_prop.equals("area"))
            {
                result=String.valueOf(PI*(radius*radius))+" "+metric+"^2";
            }
            else
            {
                result=String.valueOf(2*PI*radius)+" "+metric;
            }
            
            out.println("<h2 style='color:orange'>"+circle_prop+" : "+result+"</h2>");
